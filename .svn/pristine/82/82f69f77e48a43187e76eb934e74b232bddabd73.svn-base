...
export default function (nav, options = {}) {
  ...
  let footerHeight = options.footerHeight || 0
  let height = options.height || 0
  let tabPanel=nav.parentNode.nextElementSibling
  ...
  tabPanel.removeEventListener('scroll', tabPanel.e)
  ...
  const scrollHandler = function () {
    ...
    if (distance > navOffsetY) {
      tabPanel.e = scrollSmall
      tabPanel.addEventListener('scroll', scrollSmall)
      tabPanel.style.height =(getHeight()-offset-height-footerHeight)+ 'px'
  ...
  const scrollSmall = function () {
    const distance = tabPanel.scrollTop
    if(distance<0){
        nav.classList.remove('vux-fixed');
        tabPanel.style.height ='auto'
    }else{
